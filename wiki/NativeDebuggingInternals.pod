...
Each record uniquely identifies a single request.
   ...
   * application: which application this goes with.  Same as the C<ID> parameter in the ApacheConfiguration.
...
Each record contains the data related to single call from one of the L<Debugging Methods|DebuggingTools#Methods>.
