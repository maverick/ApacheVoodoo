...
Each Voodoo application has it's own configuration file that is kept in C<E<lt>ServerRootE<gt>/sites/E<lt>name_of_applicationE<gt>/etc/voodoo.conf>
...
=head1 Example Configuration
	...
	<database>
	    connect  C< "dbi:mysql:database>my_app;hostC<127.0.0.1;mysql_connect_timeout>2"
	    username = "test_user"
...
=head1 ViewHTML Options

=head2 template_opts

Key value pairs passed to the new of L<HTML::Template|CPAN:HTML::Template>.  See ViewHTML for more details.
...
Which view to use by default.  Defaults to L<HTML|ViewHTML>.
...
Any other parameters are accessible to the controllers via L<$p|ControllerAPI>->{'template_conf'}.  The L<HTML view|ViewHTML> also passes these to the underlying template.
