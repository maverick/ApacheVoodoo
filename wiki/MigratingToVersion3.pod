...
=head2 Removed from 3.x
   ...
   * CPAN:Apache::DBI (superseded by DBI-E<gt>connect_cached)
...
and then either inside a C<E<lt>PerlE<gt>> configuration block or your C<modperl-startup.pl> file add
...
=head1 Using the New Debugging Tools
 ...
 The new debugging tools support multiple debugging backends, Native (V2 style), FirePHP, and Log4perl. You can can mix and match which debugging system you want to use and control the behavior of each individually. As a side effect, the C<debug> flag in C<voodoo.conf> is no longer a boolean, but a block. For sake of backwards compatibility, C<debug>1C< enables Native and FirePHP in log everything mode, >debugC<0> disable all debugging.
...
Native debugging does this by passing off the data to a separate logging daemon which writes it to either to a SQLlite or MySQL database. A separate apache handler handles the display of the captured data.

=head3 Configuring Voodoo

 First voodoo needs to told how where the debug database is going to stored. Execute C<voodoo-control setconfig> and set the values of C<Debug DB Connection>, C<Debug DB Username>, C<Debug DB Password>, and C<Debug URL Path>. The first three are the parameters are passed to C<DBI-E<gt>connect> to establish the connection the debugging database. C<Debug URL Path> tells the debug panel where on the server the debug handler is setup. For example:
...
uses a local SQLite database to store the debugging information. If you use MySQL for storage, give each instance of Apache its database; don't try sharing the same one.

=head3 Start the logging daemon.

 Start the logging daemon by executing C<voodoo-daemon start>, it should be in the same directory as C<voodoo-control>. Be sure to do this *before* you start Apache, or at the very least before the first request. The Native debugging module will automatically disable itself if it can't contact the logging daemon.
