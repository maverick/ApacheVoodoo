...
Triggers the initialization of Voodoo on Apache start or restarts.  This let's us use 'graceful'
restarts with apache.  

You need to have mod_perl compiled with support for the Restart Handler.  Some pre-compiled
versions of mod_perl do not include it.  If you get an error about PerlRestartHandler being
and invalid directive, that's the culprit.
...
This let's Voodoo know what application is attached to this directory.  This also corresponds
to the name of the Voodoo configuration file, and base package name (unless overridden) of
your application.  
