...
$p is a hash reference that contains plethora of things:
    ...
    dbh           => DBI object already connected to the database as described in the Voodoo configuration file
	dir_config    => Directory configuration directives from Apache (see the mod_perl docs for $r->dir_config)
	document_root => /path/to/the/document/root/for/this/site
	params        => key / value pairs of all the url POST or GET parameters
	session       => Apache::Session object where you may store any client persistant data
	template_conf => Contents of the matching template_conf section in the Voodoo configuration file
	uri           => /the/current/url/minus/the/server.com/part/and/parameters
	user-agent    => User-agent string as presented by the browser
