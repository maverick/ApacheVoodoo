...
Triggers the initilization of Voodoo on Apache start or restarts.  This let's us use 'graceful' restarts
with apache.  

You need to have mod_perl compiled with support for the Restart Handler.  Some pre-compile versions of
mod_perl do not include it.  If you get a error about PerlRestartHandler being and invalid directive, 
that's the culprit.
...
and writeable by whatever user Apache runs as.
