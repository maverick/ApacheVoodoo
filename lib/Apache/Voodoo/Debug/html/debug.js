...
function voodooDebug(opts) {
	...
	this.session_id = this.applications[0].session_id;
	this.request_id = this.applications[0].request_id;
	...
	this.changeApplication = function(obj) {
		this.app_id     = this.applications[obj.value -1].app_id;
		this.session_id = this.applications[obj.value -1].session_id;
		this.request_id = this.applications[obj.value -1].request_id;
		this.origin_id  = this.request_id
	...
	this.handleSection = function(obj, section) {
		if (obj.parentNode.className == "vdOpen") {
			...
			if (section != "top") {
				...
				var select = document.getElementById("voodooChangeRequest");
				if (select.length < 1) {
	...
	this.dumpData = function(data,closed) {
		...
		if (data === null) {
			...
			for (var i=0; i < keys.length; i++) {
				a.push('<li>' + keys[i] + ' => ' + this.dumpData(data[keys[i]]));
			}
			...
			var d = String(data);
			return '"' + d.replace(/"/g,'\\"').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/\n/g,"<br>").replace(/ /g,'&nbsp;') + '"';
		}
	};
