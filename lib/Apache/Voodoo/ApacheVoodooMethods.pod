...
Prints user debugging messages to the error_log file and to the 'Debug' section of the DEBUG template block. (See L<Apache::Voodoo::DebuggingTools> for more details).  Messages can be turn on or off globally using the C<debug> option of the Voodoo configuration file.
...
complexity.  Tardis provides a convenient way search the history for a set of URI's and redirect the user back to 
the first one it finds.

The uri's must contain the full path, relative uri's don't work.  You do not need to specify the parameters for 
